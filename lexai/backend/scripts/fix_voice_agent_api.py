"""Update voice_agent.py to use the correct Piper synthesis API."""
...
content = textwrap.dedent("""\
    ...
    async def synthesize(text: str, voice: str = "default") -> bytes:
        ...
        for chunk in tts.synthesize(text):
            # chunk.audio is actually the raw PCM bytes
            chunks.append(np.frombuffer(chunk.audio, dtype=np.int16))
...
pathlib.Path("agents/voice_agent.py").write_text(content, encoding="utf-8")
print("voice_agent.py fixed with Piper synthesize() OK")
